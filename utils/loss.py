...
class Logit_Interaction_Loss2(nn.Module):
    def __init__(self):
        super().__init__()
        self.mse = nn.MSELoss(reduction='none')  # 后面便于加权

    def forward(self, out_A, out_B, out_change, depth_A=None, depth_B=None):
        # 先计算类别概率
        logit_A = F.softmax(out_A, dim=1)  # [B, C, H, W]
        logit_B = F.softmax(out_B, dim=1)
        logit_Change = F.softmax(out_change, dim=1)  # 变化概率，假设2通道：unchanged, changed

        # 语义预测取最大类别索引
        pred_A = torch.argmax(logit_A, dim=1)  # [B, H, W]
        pred_B = torch.argmax(logit_B, dim=1)

        # 创建变化和不变的位置mask
        unchange = (pred_A == pred_B).float()  # float方便后续计算
        change = 1 - unchange

        # 计算语义分类的最大概率均值，表示语义变化置信度
        max_A, _ = torch.max(logit_A, dim=1)  # [B, H, W]
        max_B, _ = torch.max(logit_B, dim=1)
        SCD_logit = 0.5 * (max_A + max_B)  # 语义变化侧置信度

        # 计算深度变化概率辅助项
        if depth_A is not None and depth_B is not None:
            depth_diff = torch.abs(depth_A - depth_B)  # [B, H, W]，假设深度形状正确
            # 归一化到0~1，防止数值过大过小影响训练
            depth_min = depth_diff.view(depth_diff.size(0), -1).min(dim=1)[0].view(-1,1,1)
            depth_max = depth_diff.view(depth_diff.size(0), -1).max(dim=1)[0].view(-1,1,1)
            depth_norm = (depth_diff - depth_min) / (depth_max - depth_min + 1e-8)

            # 通过Sigmoid平滑转换成概率表示，threshold和斜率可调
            depth_change_prob = torch.sigmoid(10*(depth_norm - 0.1))  # 可调参数
        else:
            # 若无深度信息，设置为全0影响
            depth_change_prob = torch.zeros_like(logit_Change[:, 0, :, :])

        # 结合变化检测输出的变化概率和深度变化概率
        # 假设out_change第1通道为“changed”，第0为“unchanged”
        combined_change_prob = 0.5 * logit_Change[:, 1, :, :] + 0.5 * depth_change_prob
        combined_unchange_prob = 1 - combined_change_prob

        # 构造改进后的变化概率图
        CD_logit_improved = combined_unchange_prob * unchange + combined_change_prob * change

        # MSE损失，detach 防止梯度误传
        mse_map = self.mse(SCD_logit, CD_logit_improved.detach())
        loss = mse_map.mean()
