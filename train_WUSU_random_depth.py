...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--Net_name", type=str, default="WUSU_proposed")
...
class Trainer:
    ...
    ###     VSCode tensor print setting     ###
        ...
        # self.TCL = TemporalConsistencyLoss()
    ...
    def training(self, epoch):
        ...
        for i, (img1, img2, img3, mask1, mask2, mask3, mask_bn, id) in enumerate(tbar):
            ...
            #  不加逻辑一致性损失用这个
            out, out_bn, pair = self.model(img1, img2, img3)    #[b,6,512,512],[b,6,512,512],[b,512,512]
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            for img1, img2, img3, mask1, mask2, mask3, mask_bn, id in tbar:
                ...
                out = [torch.argmax(out[0], dim=1).cpu().numpy(), torch.argmax(out[1], dim=1).cpu().numpy(), torch.argmax(out[2], dim=1).cpu().numpy()]
