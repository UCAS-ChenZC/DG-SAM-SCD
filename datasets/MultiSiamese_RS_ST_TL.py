...
def read_image(image_path, drop_nir=True):
    ...
    if nBandCount == 1:  # 灰度图
        ...
        if drop_nir:
            img_data = img_data[:3, :, :]   # 保留 Red, Green, Blue，舍弃 NIR
        dataset = None
        return img_data
    else:
        dataset = None
...
def rand_rot90_flip_MCD(img_A, img_B, img_C, depth1, depth2, depth3, label_A, label_B, label_C, label_cd):
    """
    ...
    # 对深度图进行增强
    depth1 = apply_transform(depth1)
    depth2 = apply_transform(depth2)
    depth3 = apply_transform(depth3)
    ...
    return img_A, img_B, img_C, depth1, depth2, depth3, label_A, label_B, label_C, label_cd
...
def read_RSimages(mode, rescale=False):
    ...
    depth_A_dir = os.path.join(root, mode, 'depth_1')
    depth_B_dir = os.path.join(root, mode, 'depth_2')
    depth_C_dir = os.path.join(root, mode, 'depth_3')
    # NOT Use rgb labels:
    ...
    imgs_list_A, imgs_list_B, imgs_list_C, depth_list_A, depth_list_B, depth_list_C, labels_A, labels_B, labels_C, labels_cd = [], [], [], [], [], [], [], [], [], []
    count = 0
    for it in data_list:
        # print(it)
        if (it[-4:] == '.tif'):
            ...
            depth_A_path = os.path.join(depth_A_dir, it.replace('.tif', '.png'))
            depth_B_path = os.path.join(depth_B_dir, it.replace('.tif', '.png'))
            depth_C_path = os.path.join(depth_C_dir, it.replace('.tif', '.png'))
            depth_list_A.append(depth_A_path)
            depth_list_B.append(depth_B_path)
            depth_list_C.append(depth_C_path)
    ...
    return imgs_list_A, imgs_list_B, imgs_list_C, depth_list_A, depth_list_B, depth_list_C, labels_A, labels_B, labels_C, labels_cd


class Data(data.Dataset):
    def __init__(self, mode, random_flip=False):
        self.random_flip = random_flip
        self.imgs_list_A, self.imgs_list_B, self.imgs_list_C, self.depth_list_A, self.depth_list_B, self.depth_list_C, self.labels_A, self.labels_B, self.labels_C, self.labels_cd = read_RSimages(mode)
    ...
    def __getitem__(self, idx):
        ...
        depth1 = read_image(self.depth_list_A[idx])
        depth2 = read_image(self.depth_list_B[idx])
        depth3 = read_image(self.depth_list_C[idx])
        ...
        if self.random_flip:
            img_A, img_B, img_C, depth1, depth2, depth3, label_A, label_B, label_C, label_cd = rand_rot90_flip_MCD(img_A, img_B, img_C, depth1, depth2, depth3, label_A, label_B, label_C, label_cd)
        ...
        return img_A, img_B, img_C, depth1, depth2, depth3, label_A, label_B, label_C, label_cd, img_name_A
