...
# 定义：Grad-Cam hooks
    ...
    # 提取RGB特征
        ...
        depth_diff = torch.abs(x3 - x4)
        x1_sam = self.sam_input_adapter(x1, x3)
        x2_sam = self.sam_input_adapter(x2, x4)
        self._set_depth_lora_context(x3, depth_diff)
        x11, x12, x13, x14 = self.encoder(x1_sam)
        self._set_depth_lora_context(x4, depth_diff)
        x21, x22, x23, x24 = self.encoder(x2_sam)
        
        x11 = self.depth_mods[0](x11, x3, depth_diff)
        x12 = self.depth_mods[1](x12, x3, depth_diff)
        x13 = self.depth_mods[2](x13, x3, depth_diff)
        x14 = self.depth_mods[3](x14, x3, depth_diff)

        x21 = self.depth_mods[0](x21, x4, depth_diff)
        x22 = self.depth_mods[1](x22, x4, depth_diff)
        x23 = self.depth_mods[2](x23, x4, depth_diff)
        x24 = self.depth_mods[3](x24, x4, depth_diff)
        
        # x1 = self.sam_input_adapter(torch.cat([x1, x3], dim=1))
        # x2 = self.sam_input_adapter(torch.cat([x2, x4], dim=1))
        # x11, x12, x13, x14 = self.encoder(x1)
        # x21, x22, x23, x24 = self.encoder(x2)
        # x11, x12, x13, x14 = self.encoder(torch.cat([x1, x3], dim=1))
        # x21, x22, x23, x24 = self.encoder(torch.cat([x2, x4], dim=1))
...
####
        ...
        xc = self.dep_change_specific_transfer(x1, x2, x3, x4)  # 引入深度信息进行变化特定特征提取
