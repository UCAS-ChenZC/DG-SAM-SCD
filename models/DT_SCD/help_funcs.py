...
class Cross_Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., softmax=True, similarity_type='tanh'):
        super().__init__()
        inner_dim = dim_head * heads
        self.heads = heads
        self.scale = dim ** -0.5
        self.softmax = softmax
        self.similarity_type = similarity_type  # 新增参数
    ...
    def forward(self, x, m, mask=None):
        ...
        if self.softmax:
            ...
            if self.similarity_type == 'tanh':
                attn = torch.tanh(dots)
            elif self.similarity_type == 'sigmoid':
...
class TransformerDecoder(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout, softmax=True,similarity_type='tanh'):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Residual2(PreNorm2(dim, Cross_Attention(dim, heads = heads,
                                                        ...
                                                        softmax=softmax,similarity_type='tanh'))),
                Residual(PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout)))
            ]))
    def forward(self, x, m, mask = None):
        """target(query), memory"""
