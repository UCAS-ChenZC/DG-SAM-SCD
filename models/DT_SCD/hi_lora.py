...
class DepthGuidedLoRA_qkv(nn.Module):
    def __init__(
        self,
        qkv,
        linear_a_q,
    ...
    def set_depth_context(self, depth, depth_diff):
        """
        depth:      B,1,H,W
        depth_diff: B,1,H,W
        """
        with torch.no_grad():
            depth = depth.detach()
            depth_diff = depth_diff.detach()

            d_mean = depth.mean(dim=[2, 3])
            d_std = depth.std(dim=[2, 3])
            dd_mean = depth_diff.mean(dim=[2, 3])
            self.depth_stats = torch.cat([d_mean, d_std, dd_mean], dim=1)

    def forward(self, x: Tensor):
        qkv = self.qkv(x)

        q_ba = (self.alpha / self.rank) * self.linear_b_q(
            self.dropout(self.linear_a_q(x))
        )
        v_ba = (self.alpha / self.rank) * self.linear_b_v(
            self.dropout(self.linear_a_v(x))
        )

        if self.depth_stats is not None:
            # depth_stats: [B, 3]
            depth_stats = self.depth_stats.to(device=x.device, dtype=x.dtype)

            # gate: [B, C]
            gate = self.depth_gate(depth_stats)

            # 兼容 SAM2-Hiera 的 window attention:
            # q_ba 可能是 [B, H, W, C]，也可能是 [B*num_windows, Wh, Ww, C]
            q_batch = q_ba.shape[0]
            g_batch = gate.shape[0]

            if q_batch != g_batch:
                if q_batch % g_batch == 0:
                    repeat_times = q_batch // g_batch
                    gate = gate.repeat_interleave(repeat_times, dim=0)
                else:
                    raise RuntimeError(
                        f"Depth gate batch size mismatch: "
                        f"q_ba.shape={q_ba.shape}, gate.shape={gate.shape}"
                    )

            # 根据 q_ba 维度自动 reshape
            # SAM2 Hiera 通常是 [B_or_BW, H, W, C]
            if q_ba.dim() == 4:
                gate = gate.view(q_batch, 1, 1, self.d_model)
            # 兼容 [B_or_BW, N, C]
            elif q_ba.dim() == 3:
                gate = gate.view(q_batch, 1, self.d_model)
            else:
                raise RuntimeError(f"Unsupported q_ba shape: {q_ba.shape}")
...
class LoRA_sam(nn.Module):
    ...
    def __init__(self, sam_model, rank: int, lora_layer=None):
        ...
        for t_layer_i, blk in enumerate(sam_model.encoder.blocks):
            ...
            if t_layer_i % 2 == 0:
                ...
                blk.attn.qkv = lora_qkv
                self.depth_lora_modules.append(lora_qkv)
    ...
    def set_depth_context(self, depth, depth_diff):
        for m in self.depth_lora_modules:
