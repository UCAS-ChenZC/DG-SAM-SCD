...
from models.DT_SCD.BTSCD_1208_sam_WUSU import BTSCD_WUSU as Net
from utils.palette import color_map
from utils.metric import IOUandSek
from utils.loss import ChangeSimilarity, DiceLoss, Logit_Interaction_Loss2
import os
import numpy as np
import torch
...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--Net_name", type=str, default="DG_TCL_NoNIR")
        parser.add_argument("--backbone", type=str, default="Resnet34")
        parser.add_argument("--data_root", type=str, default="/home/solid/CD/datasets/SCD_data/GSTM/WUSU_data")
...
class Trainer:
    ...
    ###     VSCode tensor print setting     ###
        ...
        _ = LoRA_sam(model, 16) # LoRA rank设置为16
        self.model = model
    ...
    def training(self, epoch):
        ...
        for i, (img1, img2, img3, depth1, depth2, depth3, mask1, mask2, mask3, mask_bn, id) in enumerate(tbar):
            running_iter = curr_iter + i + 1
            img1, img2, img3 = img1.float().cuda(), img2.float().cuda(), img3.float().cuda()
            depth1, depth2, depth3 = depth1.float().cuda(), depth2.float().cuda(), depth3.float().cuda()
            mask1, mask2, mask3, mask_bn = mask1.cuda().long(), mask2.cuda().long(), mask3.cuda().long(), mask_bn.cuda().long()
            #  不加逻辑一致性损失用这个
            # out1, out2, out3, out_bn, loss_sim = self.model(img1, img2, img3, depth1, depth2, depth3)    #[b,6,512,512],[b,6,512,512],[b,512,512]
            ...
            # loss_TL = self.TCL(
            #         F.softmax(out1 / self.TCL.temp, dim=1),
            #         F.softmax(out2 / self.TCL.temp, dim=1),
            #         F.softmax(out3 / self.TCL.temp, dim=1)
            #     )
            loss_TL = self.TCL(
                    self.softmax(out1),
                    self.softmax(out2),
            ...
            ##
            # loss_kd = self.kd(out1, out3, out_bn)
            # loss = loss_bn + loss_seg + loss_similarity + loss_TL + 0.5*(loss_kd + loss_sim)
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            for img1, img2, img3, depth1, depth2, depth3, mask1, mask2, mask3, mask_bn, id in tbar:
                img1, img2, img3 = img1.float().cuda(), img2.float().cuda(), img3.float().cuda()
                depth1, depth2, depth3 = depth1.float().cuda(), depth2.float().cuda(), depth3.float().cuda()

                out1, out2, out3, out_bn13, _ = self.model(img1, img2, img3, depth1, depth2, depth3)
                ...
                out_bn = torch.argmax(out_bn13, dim=1).cpu().numpy().astype(np.uint8)
                out1[out_bn == 0] = 0
                out3[out_bn == 0] = 0
                mask1[mask_bn == 0] = 0
                mask3[mask_bn == 0] = 0
