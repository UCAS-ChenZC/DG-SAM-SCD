...
from datasets.change_detection_depth import ChangeDetection_SECOND
# from datasets.change_detection import ChangeDetection_WUSU
...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--Net_name", type=str, default="D_SAM_test_NoDA")
        parser.add_argument("--backbone", type=str, default="resnet34")
        parser.add_argument("--data_root", type=str, default="/home/solid/CD/datasets/SCD_data/SECOND_908")
        # parser.add_argument("--data_root", type=str, default=r"/Long-term-SCD/wusu512_process")
...
class Trainer:
    ...
    ###     VSCode tensor print setting     ###
        ...
        # ================= 检查 LoRA 是否注入成功 =================
        num_depth_lora = sum(
            1 for m in self.model.encoder.modules()
            if hasattr(m, "set_depth_context")
        )

        print("\n========== Depth-guided LoRA Check ==========")
        print("Depth-guided LoRA modules:", num_depth_lora)

        trainable = [(n, p.numel()) for n, p in self.model.named_parameters() if p.requires_grad]
        print("Trainable parameter groups:", len(trainable))
        print("Trainable params:", sum(x[1] for x in trainable))

        print("\nFirst 20 trainable params:")
        for n, c in trainable[:20]:
            print(n, c)

        print("\nTrainable LoRA / depth_gate params:")
        for n, p in self.model.named_parameters():
            if p.requires_grad and (
                "linear_a" in n or "linear_b" in n or "depth_gate" in n
            ):
                print(n, p.numel())

        print("============================================\n")
        frozen_encoder_params = 0
        trainable_encoder_params = 0
        trainable_non_lora_encoder = []

        for n, p in self.model.encoder.named_parameters():
            if p.requires_grad:
                trainable_encoder_params += p.numel()
                if not (
                    "linear_a" in n or
                    "linear_b" in n or
                    "depth_gate" in n
                ):
                    trainable_non_lora_encoder.append(n)
            else:
                frozen_encoder_params += p.numel()

        print("\n========== Encoder Freeze Check ==========")
        print("Frozen encoder params:", frozen_encoder_params)
        print("Trainable encoder params:", trainable_encoder_params)
        print("Unexpected trainable non-LoRA encoder params:", len(trainable_non_lora_encoder))
        for n in trainable_non_lora_encoder[:30]:
            print(n)
        # ===========================================================
